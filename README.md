...
| Item | Status | Target |
|---|---|---|
| **MSSQL upsert via `MERGE`** | INSERT / UPDATE / DELETE / SELECT work today; upsert throws `NotImplemented`. | 2.5.0 |
| **Mongo `nearTo` cross-field** | If a `near` filter targets field A and a `nearTo` orderBy targets field B, the `$geoNear` stage only honors B (single-stage limit). | 2.5.0 |
...
| **Browser bundle of `forge doctor` / `forge diff`** | `db.$migrate()` covers DDL apply in the browser today; full doctor / diff CLI parity (with structured drift output for in-app rendering) is planned. | 2.5.0 |
