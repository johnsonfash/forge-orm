# Browser (sqlite-wasm + OPFS) — full reference

Companion to the README's **[Browser chapter](../README.md#browser-sqlite-wasm--opfs)**. The README covers the one-paragraph intro and a quickstart. This file is the full reference: URL schemes, the worker file, every supported bundler's setup, `$migrate()` semantics in detail (including 2.5.1 drift detection), `browserDoctor()` shape, persistent storage and Safari ITP, multi-tab safety, the stock-vs-pro wasm feature matrix, the custom build path, and a troubleshooting cookbook.

For framework recipes (React + Vite, Next.js, Vue, Nuxt, SvelteKit, Angular, SolidStart, Astro, Remix, React Native, Tauri), see **[docs/BROWSER-FRAMEWORKS.md](./BROWSER-FRAMEWORKS.md)**.
...
* [`db.$migrate()` — runtime DDL apply + drift detection](#dbmigrate--runtime-ddl-apply--drift-detection)
...
## `db.$migrate()` — runtime DDL apply + drift detection
...
| `$queryRaw` / `$executeRaw` | ✓ | ✓ | ✓ |
| Migrations | `db.$migrate()` (runtime, drift-aware since 2.5.1) | `db.$migrate()` (runtime, drift-aware) | `forge push` (CLI) |
| Doctor probe | `browserDoctor()` / `db.$doctor()` | `browserDoctor()` / `db.$doctor()` | `forge doctor` (CLI) |
| Drift detection (`forge diff`) | `db.$diff()` + `$migrate()` auto-applies safe ALTERs | Same | `forge diff` (CLI) |
