...
would have applied it never ran on the production pipeline — somebody had
moved it a year earlier and it had been fine ever since, in the way that
things are fine right up until the first time they aren't.
...
`findMany`. The thing that actually costs money is what happens the day you
change a column on a table that already has rows in it.

So: the unglamorous half. The CLI, migrations, and the checks that catch that
night before it reaches anyone.
...
There's no `--url` flag — you point a different `DATABASE_URL` at the binary,
which makes a DB-per-tenant deploy a `for` loop.

## `forge diff --check` — the gate

`diff` is the read-only sibling: introspect, compare, print, never write. Safe
with a read-only role, so CI can preview production without write access to it.
...
The exit codes matter more than they look. `--check` exits **3** for real
drift and **1** for "couldn't run at all" — no `DATABASE_URL`, unreachable
database, schema didn't load. A job that fails identically on both trains
everyone to ignore it.

## `forge generate` — a migration with no database in the room

`forge diff apply` generates by introspecting `DATABASE_URL`. That's the right
tool for adopting a database somebody else created, and the wrong one for
everyday work:

- **CI has no database**, so nothing could verify that a schema change arrived
  with its migration.
- **Two branches generate against two worlds.** Alice generates against her
  local database on Monday, Bob against his on Tuesday. Each file is correct
  relative to a state that stops existing at the merge.
- **The same schema didn't reliably produce the same SQL**, so a reviewer
  couldn't regenerate a migration to check it against the schema change in the
  same pull request.
...
the comparison code didn't move at all.
...
schema will then be. A reviewer who sees one without the other is looking at
half the change.

## `forge migrate status` — the two states nobody reports

Everything above compares *intent* — your schema against a snapshot, or
against what a database reports. `migrate status` compares *reality*: the
files in `migrations/` against the rows in `_forge_migrations`. It's the one
command that genuinely needs `DATABASE_URL`, because a database is the only
thing that knows what it has actually run.
...
as *"why is this column missing in staging"*. The fix is to delete the file
and its snapshot, pull, and regenerate on top of the current state — which
also re-diffs it against the schema as it now is, and that's the part that
matters. `0007` was written against a world where `0008` hadn't happened.
...
is not the schema that database has, so every migration you generate from here
is built on a state you cannot see — correct against your snapshot, wrong
...
naming both columns and printing the line to add. `--allow-drop` is how you
say *"it really is a drop, I mean to lose it"* — deliberately a flag and not a
prompt, so it lands in the shell history and in whatever ran the command.

drizzle-kit asks this interactively. Right question, wrong medium: a prompt
answered once at 2am is recorded nowhere, cannot run in CI, is invisible in
review, and has to be answered again by the next person who regenerates. An
annotation is in the schema, the diff and the pull request.

Only same-typed pairs are suspected. A dropped `text` column beside a new
`integer` one isn't a candidate — a check that fires on every genuine drop is
one people learn to click past, and then it protects nothing.

### A widening is emitted; a narrowing is refused

`varchar(64)` → `varchar(255)`, `int` → `bigint`, dropping `NOT NULL`: every
existing row still fits, so the statement cannot fail on data. Emitted — with
a `down` that says out loud that reversing it can fail on rows written since.
A file that says "rollback" without saying that is lying to whoever runs it at
3am.
...
`NULL` → `NOT NULL` is always refused too, and the message says why the
obvious fix doesn't work: a `DEFAULT` applies to new rows, not to the NULLs
already there.
...
`DATABASE_URL` shape check, a schema lint for impossible index combinations,
and a live probe that tells you `pg_trgm` isn't installed before a GIN index
tells you at deploy time.
...
pointing squarely at the one part of the statement that was fine. An empty set
now emits a column assigned to its own stored value: valid SQL, provably no
change, and `RETURNING` still yields the row, which upsert's contract requires.
`DO NOTHING` would have parsed and is the shorter fix, but it returns no row on
conflict, so `upsert` would have resolved to `undefined` exactly when the
record already existed. A silent wrong answer in place of a loud syntax error
is not a fix.
...
Both of those were found by *running* an example rather than reading one. Which
is the embarrassing bit: the examples lived in their own repository, pinned at
`^2.5.6` while the library reached 2.13.0, and three of them were broken. One
was broken by the empty upsert above — a bug nothing caught for **eight
releases**, because the only code exercising that path was in a repository no
CI was running.

They live in the main repo now and run against the working tree on every push
and pull request, so a change that breaks an example fails the build that broke
it. The whole post is about the gap between "this is fine" and "something
checks that this is fine", and I was standing in it.
...
clause and one that meant to change something. In each case a differ could
have guessed, been right most of the time, and been catastrophically wrong the
rest — and the wrongness would not have surfaced as an error. It surfaces as an
empty column, a foreign key pointing nowhere, an upsert returning `undefined`,
a migration nothing ever applied.

I've come round to thinking that's the real measure of a data tool. Not the
query API; everyone's query API is fine now, they converged on roughly the same
shape and it took me a while to admit it. The measure is what a tool does at
the boundary of its own knowledge.

An error costs you a minute. A silent wrong answer costs you a quarter, and
you don't find out which quarter until the accounting doesn't add up.
...
**Part 1 — the day-one database decision.** *"I picked MongoDB on day one. By
month eight I needed Postgres."*

**Part 2 — the codegen step.** *"The afternoon my teammate lost to a stale
generated client."* Inferred types instead of a generated client, and
`$explain` — seeing the query without running it.

**Part 3 — the browser.** The same models and the same `findMany` in a tab,
against SQLite compiled to WebAssembly.

**Part 4 — React Native.** What "no native module of our own" is worth on a
platform where every native module is a rebuild.
